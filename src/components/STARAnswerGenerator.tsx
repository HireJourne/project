...
import { generateBehavioralSTAR, STARAnswer } from '../services/starService';
...
const STARAnswerGenerator: React.FC<Props> = ({ parsedResume, parsedJD }) => {
  ...
  const handleGenerate = async () => {
    try {
      setLoading(true);
      const generatedAnswers = await generateBehavioralSTAR(parsedResume, parsedJD);
      setAnswers(generatedAnswers);
    } catch (error) {
      console.error('Failed to generate STAR answers:', error);
      toast.error('Failed to generate interview answers');
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        <div>
          <h2 className="text-2xl font-bold text-gray-800">STAR Interview Answers</h2>
          <p className="text-gray-600 mt-1">
            Generate behavioral interview answers in STAR format
          </p>
        </div>
            ...
            >
              ...
              {expandedIndex === index && (
                <div className="px-6 py-4 border-t space-y-4">
                  ...
                  <div>
                    <h4 className="font-medium text-gray-700">Task</h4>
                    <p className="text-gray-600 mt-1">{answer.star_i_answer.task}</p>
                  </div>
                  
                  <div>
                    <h4 className="font-medium text-gray-700">Action</h4>
                  ...
                  <div className="border-t pt-4">
                    <h4 className="font-medium text-primary">Impact & Application</h4>
